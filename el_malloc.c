...
el_blockhead_t *el_get_header(el_blockfoot_t *foot) {
    ...
    el_blockhead_t *head = PTR_MINUS_BYTES(foot, sizeof(el_blockfoot_t) + size);
    return head;
}
...
el_blockhead_t *el_block_below(el_blockhead_t *block) {
    ...
    el_blockfoot_t *decr_foot = PTR_MINUS_BYTES(block, sizeof(el_blockfoot_t));

    if ((void *) decr_foot <= (void*) el_ctl.heap_start) {
        return NULL;
    } 

    el_blockhead_t *decr_head = PTR_MINUS_BYTES(decr_foot, decr_foot->size + sizeof(el_blockhead_t)); 
...
el_blockhead_t *el_find_first_avail(size_t size) {
    ...
    el_blockhead_t *found = el_ctl.avail->beg;

    for (int i = 0; i < el_ctl.avail->length; i++){
	
	found = found->next;

	if (found->size >= (size + EL_BLOCK_OVERHEAD) && found->state == EL_AVAILABLE){
	    return found;
	    }
    }
    return NULL;
...
el_blockhead_t *el_split_block(el_blockhead_t *block, size_t new_size) {
    ...
    if (block->size < (new_size + EL_BLOCK_OVERHEAD)){     //Checks for NULL
	return NULL;
	}

    size_t available_size = block->size - (new_size + EL_BLOCK_OVERHEAD);

    block->size = new_size;  //make new size and state for old head
    el_blockfoot_t *new_foot = el_get_footer(block); //create and assign new foot to used head
    new_foot->size = new_size;

    el_blockhead_t *new_head = el_block_above(block); //creates new block for available space
    new_head->size = available_size;

    el_blockfoot_t *old_foot = el_get_footer(new_head); // Assign the old foot to the new head
    old_foot->size = available_size;
...
void *el_malloc(size_t nbytes) {
    ...
    el_blockhead_t *old_head = el_find_first_avail( nbytes);

    if (old_head == NULL){
	return NULL;
	}
    el_remove_block(el_ctl.avail, old_head);

    el_blockhead_t *new_head = el_split_block(old_head, nbytes);

    new_head->state = EL_AVAILABLE;

    el_blockhead_t *lower = old_head;
    lower->state = EL_USED;
    
    el_add_block_front(el_ctl.used, lower);
    el_add_block_front(el_ctl.avail, new_head);

    return PTR_PLUS_BYTES(lower, sizeof(el_blockhead_t));
}
...
void el_merge_block_with_above(el_blockhead_t *lower) {
	...
	if (upper != NULL && upper->state == EL_AVAILABLE){
	    ...
	    size_t size = lower->size + upper->size + EL_BLOCK_OVERHEAD;
	    lower->size = size;
    ...
    if (el_block_below(lower) != NULL && el_block_below(lower)->state == EL_AVAILABLE){
		el_blockhead_t *below_lower = el_block_below(lower);
		el_merge_block_with_above(below_lower); //checks for blocks below lower
	    }
